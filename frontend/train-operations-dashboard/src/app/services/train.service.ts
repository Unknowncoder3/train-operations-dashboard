...
export class TrainService {
  ...
  private readonly fallbackTrains: Train[] = [
  ...
  private readonly fallbackAlerts: OperationalAlert[] = [
  ...
  getTrains(): Observable<Train[]> {
    return this.http.get<Train[]>(`${this.apiUrl}/trains`).pipe(catchError(() => of(this.fallbackTrains)));
  }

  getSummary(): Observable<DashboardSummary> {
    return this.http.get<DashboardSummary>(`${this.apiUrl}/dashboard/summary`).pipe(
      catchError(() => of({ totalTrains: 1248, activeTrains: 1234, delayedTrains: 127, cancelledTrains: 14, onTimePercentage: 89.8 }))
    );
  }

  getAlerts(): Observable<OperationalAlert[]> {
    return this.http.get<OperationalAlert[]>(`${this.apiUrl}/alerts`).pipe(catchError(() => of(this.fallbackAlerts)));
  }
}
